...
class Injector {
  constructor() {
    this.inject = (alias, css) => {
      let head = document.head || byTag("head")[0],
    ...
    this.remove = (alias) => {
      let style = getById('injector:' + alias);
      if (style) {
        let head = document.head || byTag('head')[0];
        if (head) {
...
class Gs {
  constructor() {
    this.init = on => {
      let hiddenReplies = selectAll('article[id^=p].deleted').length,
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  let on = +(localStorage['GS_showHiddenPosts'] || 1);
